...
%               first time.
%
delta3 = h - Y; % num_labels, m
delta2 = Theta2(:, 2:end)' * delta3 .* sigmoidGradient(z2); % hidden_layer_size, m
Theta1_grad = delta2 * a1 ./ m; % hidden_layer_size, input_layer_size + 1
Theta1 = [zeros(size(Theta1, 1), 1) Theta1(:, 2:end)];
Theta1_grad = Theta1_grad + Theta1 .* lambda ./ m;
Theta2_grad = delta3 * a2' ./ m; % num_labels, hidden_layer_size + 1
